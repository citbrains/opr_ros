...
void	joy_init( void )
{
	...
	is_walk_change					=	0;		// 歩行を変更したかどうかのフラグ
}
...
//##############################################################################
		...
		case	'A':		/*	walk all direction	*/
			...
			/***	number of steps	***/
			if( !sq_flag.walk ) xv_mv.count = 0;	// 歩行していない時には，歩数カウンタを0にする．
			...
			sq_flag.walk		=	ON;
			is_walk_change		=	1;				// 歩行を変更したかどうかのフラグ
		...
		case	'C':		/*	cancel	*/
			...
			if( _xv_comm_bin.para1 == 0 )
			{
				if(mode_motion != MOTION_MOTION) // only stop if current motion is not special action
				{
					...
					reset_flag(sq_flag);		// 全ての状態フラグをクリア
					xv_joy.walk_x_percent		=	0.f;
					xv_joy.walk_y_percent		=	0.f;
					xv_joy.walk_theta_percent	=	0.f;
